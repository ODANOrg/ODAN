...
type Intent = 'help' | 'volunteer' | 'both' | null;
...
export default function RegisterPage() {
  ...
  const postAuthRedirect = useMemo(() => {
    if (intent === 'help') return '/tickets/new';
    if (intent === 'volunteer') return '/dashboard/volunteer';
    return '/dashboard';
  }, [intent]);

  const handleRegister = (provider: string) => {
    try {
      localStorage.setItem('odan-post-auth-redirect', postAuthRedirect);
    } catch {
      // ignore
    }
      ...
      <div className="container grid grid-cols-1 gap-6 px-4 py-6 sm:gap-8 sm:px-6 sm:py-10 lg:grid-cols-[1.05fr_0.95fr] lg:items-center lg:gap-12 lg:py-12">
        <div className="flex w-full max-w-full flex-col items-center justify-center space-y-4 text-center text-white sm:space-y-6 lg:items-start lg:text-left">
          ...
          <div className="space-y-3">
            <h1 className="text-2xl font-semibold leading-tight sm:text-4xl md:text-5xl">{t('heroTitle')}</h1>
            <p className="max-w-full text-base text-white/80 sm:max-w-2xl sm:text-lg">{t('heroSubtitle')}</p>
          </div>
          ...
          <div className="grid w-full max-w-full grid-cols-1 gap-4 sm:max-w-2xl sm:grid-cols-2 lg:max-w-none">
            <Card className="bg-white/5 border-white/10 text-white">
              <CardHeader className="pb-2">
                ...
                <CardDescription className="text-white/70">{t('cards.user.subtitle')}</CardDescription>
              </CardHeader>
            ...
            <div className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
                <Button
                  ...
                  onClick={() => setIntent('help')}
                >
                  {t('roleHelp')}
                </Button>
                <Button
                  className="w-full"
                  variant={intent === 'volunteer' ? 'default' : 'outline'}
                ...
                <Button
                  className="w-full"
                  variant={intent === 'both' ? 'default' : 'ghost'}
